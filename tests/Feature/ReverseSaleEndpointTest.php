...
/*
|--------------------------------------------------------------------------
| POST /pos/sale/{receiptNo}/reverse — cashier endpoint-i.
|--------------------------------------------------------------------------
*/
...
it('returns 404 for a receipt that belongs to another merchant (no enumeration)', function () {
    ...
    // Cari merchant-ın kassiri reverse etməyə çalışır.
...
it('requires return_receipt_no', function () {
    ...
    expect(Transaction::where('receipt_no', 'r_rev_ep_5')->value('status'))->toBe('completed');
});
...
it('blocks non-cashier roles (e.g. customer)', function () {
